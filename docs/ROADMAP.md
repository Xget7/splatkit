...
The cull margin is 10 degrees plus the turn rate times the time a cull takes to reach the screen (50 ms at least, more when the frame or the cull is slow), so a flick of the phone finds its edges already drawn; the first version re-sorted the culled set with a fixed margin and showed empty edges on fast turns, and a fixed 50 ms showed them again on a 3.5M scene at 15 fps.
