...
The renderer keeps two frames in flight, writes each new draw order into the buffer the frame in flight is not reading, and reads GPU time from the command buffer.
